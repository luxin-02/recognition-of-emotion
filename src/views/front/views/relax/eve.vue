<template>
  <div class="index">
    ...
    <div class="light">
      <img class="ding" src="@/assets/img/front/relax/ding.png" />
      <img
        class="deng"
        src="@/assets/img/front/relax/deng.png"
...
export default {
  data() {
    return {
      currentAngle: 0, // 当前摆动角度
      swingSpeed: 5, // 摆动速度（1-10）
      swingAmplitude: 80, // 摆动幅度
      animationId: null,
      startTime: 0, // 动画开始时间
      lastTime: 0, // 上一帧时间
      drillTime: 0, // 训练时间（秒）
  ...
  methods: {
    ...
    startSwinging() {
      ...
      const animate = (currentTime) => {
        ...
        // 根据速度计算频率（速度越快，频率越高）
        const frequency = 0.1 + (this.swingSpeed - 1) * 0.05 // 0.5到1.85Hz

        // 使用正弦函数实现平滑摆动
        // 角度 = 幅度 * sin(2π * 频率 * 时间)
        this.currentAngle =
          this.swingAmplitude * Math.sin(2 * Math.PI * frequency * totalTime)
...
.index {
  ...
  .light {
    ...
    .deng {
      ...
      top: 33px; // 与钉子同一高度
      left: 44.9%;
      transform-origin: 50% 0; // 旋转中心在顶部中间（钉子位置）
