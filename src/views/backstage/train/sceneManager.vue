<template>
  <div class="class-box">
    <div class="title-box">场景管理分类及内容列表</div>
    <!-- 搜索 -->
    ...
    <div class="add-del_change">
      <buttonVue v-auth="'/index/music/addclass'" @clickBtn="()=>{addClassName='';showAddClass = true}"
                 :iconUrl="require('@/assets/img/backstage/assembly/btnAdd.png')"
                 backgroundColor="#00AEFF" text="新增分类"></buttonVue>
      <buttonVue v-auth="'/index/music/delclass'" :iconUrl="require('@/assets/img/backstage/assembly/btnDelete.png')"
                 backgroundColor="#FF7B2B"
                 text="删除分类" @clickBtn="removeClass">
      </buttonVue>
      <buttonVue v-auth="'/index/music/add'" :iconUrl="require('@/assets/img/backstage/assembly/btnAdd.png')"
                 backgroundColor="#2E8AE6"
                 text="新增场景" @clickBtn="openAdd"></buttonVue>
      <buttonVue v-auth="'/index/music/edit'" text="修改" @clickBtn="openEdit"></buttonVue>
      <buttonVue v-auth="'/index/music/del'" text="删除" @clickBtn="remove"></buttonVue>
    </div>
    <div class="total">当前列表共有数据：{{ page.total }}条</div>
    <div class="table">
      <ClassChange value_key="id" label="title" :title="'场景分类'" v-model="page.music_class_id"
                   :array="audioClass"></ClassChange>
      <div class="right_box">
        ...
        <div class="audio_list">
          ...
          <div class="item" v-for="(item,i) in tableConfig.tableData">
            <div class="row">
    ...
    <backstage-dialog :isSaveLoading="addOrEditLoading||uploadAudioLoading||uploadFileLoading" :isAutoSaveClose="false"
                      class="add_edit_warp"
                      :show.sync="showAdd" title='新增/编辑场景' @save="save">
      <div class="title-box">
        <span>场景名称:</span>
      ...
      <div class="add-admin">
        ...
        <div class="add-select">
          <selectVue :options="audioClass" v-model="addOrEditForm.music_class_id" ItemValue="id"
                     label="title"></selectVue>
      ...
      <div class="add-admin flex_start">
        <span>场景切换:</span>

        <div class="add_row col">
          <div class="row">
            <div class="img_box" v-for="(item,i) in  addOrEditForm.imgList">
              <img class="upload_img" :src="$ip+item"/>
              <img @click="addOrEditForm.imgList.splice(i,1)" src="@/assets/img/backstage/popup/ic-close.png"
                   class="close"/>
            </div>
          </div>
          <div class="row">
            <img class="upload_img" src="@/assets/img/backstage/train/img_add.png"/>
            <div class="add_col">
              <div class="tips">说明：切换图片格式为.jpg或.png，尺寸建议960px*540px。</div>
              <div class="button">
                <uploadButton accept="image/png, image/jpeg" @change="changeUploadFile" v-loading="uploadFileLoading">
                  本地上传
                </uploadButton>
              </div>
            </div>
      ...
      <div class="add-admin">
        ...
        <div class="add_row">
          <div class="upload_row">
            <a class="text" v-if="addOrEditForm.music_url" :href="$ip+addOrEditForm.music_url">{{
                addOrEditForm.music_url ? addOrEditForm.music_url.split('/').pop() : ''
              }}</a>
            <div v-else class="text"></div>
            <div class="button">
              <UploadButton accept=".mp3" v-loading="uploadAudioLoading" @change="changeUploadAudio">本地上传
              </UploadButton>
            </div>
...
export default {
  async created() {
    ...
    this.audioClass.length ? this.page.music_class_id = this.audioClass[0].id : '';
  ...
  computed: {
    //组合属性用于监听搜索参数自动查询
    combinedProperties() {
      return this.page.start_time + this.page.end_time + this.page.music_class_id + this.page.title;
    },
    selectIds() {
      return this.tableConfig.tableData.filter(v => v.isCheck).map(v => v.id).join(',')
    },
    selectRows() {
  ...
  methods: {
    ...
    async saveEdit() {
      ...
      const res = await backMusicApiEdit(params)
    ...
    async saveAdd() {
      ...
      const res = await backMusicApiAdd(params)
      if (res.data.code == 200) {
        this.getTableList();
        this.showAdd = false;
    ...
    async getTableList(page) {
      ...
      if (res.data.code == 200) {
        ...
        this.tableConfig.tableData = res.data.data.data.map(v => {
          return {...v, isCheck: false, imgList: []}
        });
    ...
    async removeClass() {
      ...
      if (flag) {
        const res = await backMusicClassApiDel({ids: this.page.music_class_id})
        if (res.data.code == 200) {
          this.$message.success('删除成功！')
          await this.getAudioClassList();
          this.audioClass.length ? this.page.music_class_id = this.audioClass[0].id : this.page.music_class_id = '';
          this.getTableList();
        }
      }
    ...
    async changeUploadFile(file) {
      ...
      try {
        ...
        const data = await this.$imgCutting.asyncShow(file.raw, 960, 540);
        let cuttingFile = base64ToFile(data)
        const res = await fileApiImage(cuttingFile);
        if (res.data.code == 200) {
          this.addOrEditForm.imgList.push(res.data.data);
    ...
    async changeUploadAudio(file) {
      ...
      try {
        ...
        if (res.data.code == 200) {
          ...
          this.addOrEditForm.title ? '' : this.addOrEditForm.title = file.name;
        }
