<template>
  <div class="class-box">
    <div class="title-box">图片分类及内容列表</div>
    <!-- 搜索 -->
        ...
        <div class="table_warp">
          ...
          >
            <template v-slot:image="row">
              <img class="preview_img" :src="$ip + row.row.cover" />
            </template>
            <template v-slot:audio="row" v-if="false">
              <div class="audio_box">
                <div
                  class="icon"
                  :class="row.row.voice_url ? '' : 'error'"
                ></div>
                <div class="text">
                  {{ row.row.voice_url ? "已上传" : "未上传" }}
                </div>
              </div>
            </template>
          </tableVue>
    ...
    >
      ...
      <div class="add-admin">
        ...
        <div class="add-select">
          <selectVue
            ...
            v-model="addOrEditForm.cog_class_id"
            ItemValue="id"
            label="title"
          >
          </selectVue>
      ...
      <div class="label_title" v-if="!activeClassAuth">图片设置</div>
      <div class="tip" v-if="!activeClassAuth">
        说明：上传的图片格式为.jpg或.png，建议尺寸不小于815px*460px。
      </div>
      <div class="add-admin" v-if="!activeClassAuth">
        <span style="opacity: 0">认知图片:</span>
        <div class="add_row">
          <img
            class="upload_img"
            v-if="addOrEditForm.cover"
            :src="$ip + addOrEditForm.cover"
          />
          <img
            class="upload_img"
            v-else
      ...
      <template v-if="false">
        <div v-if="!activeClassAuth" class="label_title">音频设置</div>
        <div v-if="!activeClassAuth" class="add-admin">
          <span>上传音频:</span>
          <div class="add_row">
            <div class="upload_row">
              <a
                class="text"
                v-if="addOrEditForm.voice_url"
                :href="$ip + addOrEditForm.voice_url"
                >{{
                  addOrEditForm.voice_url
                    ? addOrEditForm.voice_url.split("/").pop()
                    : ""
                }}</a
              >
              ...
              <div class="button">
                ...
                <div class="success_tip">
                  {{ addOrEditForm.voice_url ? "已上传" : "未上传" }}
                </div>
                <UploadButton
                  accept=".mp3"
        ...
        <div v-if="!activeClassAuth" class="title-box textarea_row">
          <span>上传内容:</span>
          <textarea v-model="addOrEditForm.content"></textarea>
        </div>
      </template>
    </backstage-dialog>
  </div>
...
import {
  cognitionClassBehindApiList,
  cognitionClassBehindApiDel,
  cognitionClassBehindApiAdd,
  cognitionBehindApiList,
  cognitionBehindApiDel,
  cognitionBehindApiAdd,
  cognitionBehindApiEdit,
...
export default {
  async created() {
    ...
    this.cognitionClass.length
      ? (this.page.cog_class_id = this.cognitionClass[0].id)
      : "";
      ...
      tableConfig: {
        ...
        headerTitle: [
...

          {
            ...
            type: "function",
            callback: (row) => {
              return formatDate(new Date(row.add_time));
            },
          },
        ],
      },
  ...
  computed: {
    ...
    combinedProperties() {
      return (
        ...
        this.page.end_time +
        this.page.cog_class_id +
        this.page.title
      );
    },
    activeClassAuth() {
      let find = this.cognitionClass.find(
        (v) => v.id == this.addOrEditForm.cog_class_id
      );
  ...
  methods: {
    ...
    async saveAdd() {
      ...
      const res = await cognitionBehindApiAdd(params);
    ...
    async saveEdit() {
      ...
      const res = await cognitionBehindApiEdit(params);
      if (res.data.code == 200) {
        this.getTableList();
        this.showAdd = false;
    ...
    openAdd() {
      this.addOrEditForm = {
        cog_class_id: this.page.cog_class_id,
        title: "",
        cover: "",
        voice_url: "",
        content: "",
    ...
    async saveClass() {
      ...
      const res = await cognitionClassBehindApiAdd({
    ...
    async removeClass() {
      ...
      if (flag) {
        const res = await cognitionClassBehindApiDel({
          ids: this.page.cog_class_id,
        });
        if (res.data.code == 200) {
    ...
    //获取
    async getClassList() {
      const res = await cognitionClassBehindApiList();
      if (res.data.code == 200) {
        this.cognitionClass = res.data.data;
      }
    },
    async getTableList(page) {
      ...
      const res = await cognitionBehindApiList(params);
      if (res.data.code == 200) {
    ...
    reset() {
      let cog_class_id = this.page.cog_class_id;
      this.page = {
        title: "",
        start_time: "",
        end_time: "",
      ...
      this.page.cog_class_id = cog_class_id;
    ...
    async remove() {
      ...
      if (flag) {
        const res = await cognitionBehindApiDel({ ids: this.selectIds });
        if (res.data.code == 200) {
          this.$message.success("删除成功！");
          this.getTableList();
        }
    ...
    async changeUploadFile(file) {
      ...
      try {
        const data = await this.$imgCutting.asyncShow(file.raw, 815, 460);
        let cuttingFile = base64ToFile(data);
        const res = await fileApiImage(cuttingFile);

        if (res.data.code == 200) {
          this.addOrEditForm.cover = res.data.data;
        }
      } catch (error) {
        console.error(error);
      } finally {
    ...
    async changeUploadAudio(file) {
      ...
      try {
        ...
        if (res.data.code == 200) {
          this.addOrEditForm.voice_url = res.data.data;
